        ...
        org.apache.spark.sql.catalyst.rules.Rule<org.apache.spark.sql.catalyst.plans.logical.LogicalPlan>
{
    ...
    public LogicalPlan apply(LogicalPlan plan)
    {
        if (plan instanceof UpdateTable) {
                    ...
                    if (lp instanceof DataSourceV2Relation) {
                        ...
                        final Seq<AttributeReference> newOutput = v2Relation
                                ...
                                .map(new AbstractFunction1<AttributeReference, AttributeReference>()
                                     {
                                         @Override
                                         public AttributeReference apply(
                                                 AttributeReference v1)
                                         {
                                             return CharVarcharUtils$.MODULE$.cleanAttrMetadata(
                                                     v1);
                                         }
                                     },
                                        new CanBuildFrom<Traversable<AttributeReference>, AttributeReference, Seq<AttributeReference>>()
                                        {
                                            ...
                                            public Builder<AttributeReference, Seq<AttributeReference>> apply(
                                                    Traversable<AttributeReference> from)
                                            {
                                                return builder.apply(
                                                        from.toList());
                                            }
            ...
            Function1<LogicalPlan, LogicalPlan> mergeResolver = this::resolveMergeIntoTableIfReady;
            return plan.transformUp(PartialFunction$.MODULE$.apply(mergeResolver));
        }
